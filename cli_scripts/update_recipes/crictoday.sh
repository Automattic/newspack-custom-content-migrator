...
echo_ts "exporting SportsPress contents..."
export_staging_site_sportspress_plugin_contents
...
if [[ 1 == IS_EXPORTED_SPORTSPRESS_PLUGIN_CONTENTS ]]; then
  echo_ts 'importing SportsPress Plugin contents from the Staging site...'
  wp_cli newspack-content-migrator import-sportspress-content --input-dir=$TEMP_DIR_MIGRATOR
else echo_ts_yellow 'Skipping importing SportsPress Plugin contents from the Staging site.'; fi
