...
declare -A LIVE_SQL_DUMP_HOSTNAME_REPLACEMENTS=(
  ...
  [calmatters.org]=calmatters-newspack-1.newspackstaging.com
  [www.calmatters.org]=calmatters-newspack-1.newspackstaging.com
)
...
# ---custom vars
   LIVE_SQL_DUMP_FILE=/tmp/staging_update_2/VIP_EXPORT/06-04-20_newspack_db_users_tables_renamed.sql
   LIVE_HTDOCS_FILES=/tmp/staging_update_2/VIP_EXPORT/fake_live_files
   TEMP_DIR=/tmp/staging_update_2/tmp_update
   TEMP_DIR_MIGRATOR=$TEMP_DIR/migration_exports
   TEMP_DIR_VAULTPRESS=$TEMP_DIR/vaultpress_archive
   TEMP_DIR_VAULTPRESS_UNZIP=$TEMP_DIR_VAULTPRESS/unzip
   LIVE_SQL_DUMP_FILE_REPLACED=$TEMP_DIR/live_db_hostnames_replaced.sql
