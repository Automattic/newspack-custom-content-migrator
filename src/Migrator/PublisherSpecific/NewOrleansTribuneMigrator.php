 ...
 */
class NewOrleansTribuneMigrator implements InterfaceMigrator {
	...
	public function register_commands() {
		WP_CLI::add_command(
			'newspack-content-migrator new-orleans-tribune-albums-migration',
	...
	/**
	 * Callable for `newspack-content-migrator new-orleans-tribune-albums-migration`.
	 *
	 * @param $args
	 * @param $assoc_args
	 */
	public function new_orleans_tribune_albums_migration( $args, $assoc_args ) {
		...
		$this->validate_db_tables_exist( $wppa_tables );
		...
		$this->posts_migrator_logic->throttled_posts_loop(
			...
			function( $post ) use ( $wpdb, $wppa_source_dir, &$missing_album_photos ) {
				if ( strpos( strtolower( $post->post_content ), strtolower( '[wppa' ) ) !== false ) {
					$matches_shortcodes = $this->squarebracketselement_manipulator->match_shortcode_designations( 'wppa', $post->post_content );
					...
					foreach ( $matches_shortcodes as $match_shortcode ) {
						...
						$post_content_updated = str_replace( $shortcode_html, $gallery_block, $post->post_content );
