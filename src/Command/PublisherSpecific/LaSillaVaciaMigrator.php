...
class LaSillaVaciaMigrator implements InterfaceCommand {
	...
	public function register_commands() {
		...
		WP_CLI::add_command(
			'newspack-content-migrator la-silla-vacia-find-and-replace-images',
			[ $this, 'cmd_find_and_replace_images' ],
			[
				'shortdesc' => 'Using a log file with missing or broken images, attempt to find them or replace them with a stock image.',
				'synopsis'  => [
					[
						'type'        => 'assoc',
						'name'        => 'list-image-strings-path',
						'description' => 'Path to the file containing the list of image strings.',
						'optional'    => false,
						'repeating'   => false,
					],
					[
						'type'        => 'assoc',
						'name'        => 'processed-urls-path',
						'description' => 'Path to the file containing the list of already processed URLs.',
						'optional'    => true,
						'repeating'   => false,
					],
					[
						'type'        => 'assoc',
						'name'        => 'ignore-post-ids',
						'description' => 'Post IDs to avoid updating.',
						'optional'    => true,
						'repeating'   => false,
					],
				],
	...
	/**
	 * This command processes a text file that contains a list of image URLs/references that were found in LSV's
	 * content. If the string is a file reference, it will attempt to search for the file in the local
	 * media cache. If found, it will "upload" the file and replace the reference with the new URL.
	 *
	 * @param array $args Positional Arguments.
	 * @param array $assoc_args Associative arguments.
	 *
	 * @return void
	 */
	public function cmd_find_and_replace_images( $args, $assoc_args ) {
		$ignore_post_ids              = explode( ',', $assoc_args['ignore-post-ids'] ?? '0' );
		$ignore_post_ids_placeholders = implode( ',', array_fill( 0, count( $ignore_post_ids ), '%d' ) );

		$list_image_strings_path = $assoc_args['list-image-strings-path'];

		// $list_of_image_strings file is generated by LE and uploaded/provided manually.
		// phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents
		$list_of_image_strings = explode( "\n", file_get_contents( $list_image_strings_path ) );

		$processed_urls_path = $assoc_args['processed-urls-path'] ?? WP_CONTENT_DIR . '/processed-urls.json';

		/*
		 * Due to Atomic environment constraints, this script could get killed at any moment. So I need an easy
		 * way to track which URLs have been processed and which ones have been ignored. Writing the whole
		 * array object as a JSON file is not ideal, but it's better than serializing it. I also don't
		 * want to use the DB, because this doesn't really belong in wp_postmeta, and if I use the
		 * options table, I have to content with the get_options/update_options bug.
		 */
		$processed_urls = file_exists( $processed_urls_path ) ? (array) wp_json_file_decode( $processed_urls_path ) : [];

		$total_strings = count( $list_of_image_strings );

		global $wpdb;

		foreach ( $list_of_image_strings as $key => $image_string ) {
			echo "\n\n\n";
			ConsoleColor::white( "$key out of $total_strings" )->output();
			ConsoleColor::white( Colors::decolorize( $image_string ) )->output();

			if ( array_key_exists( $image_string, $processed_urls ) ) {
				if ( false === $processed_urls[ $image_string ] ) {
					ConsoleColor::magenta( 'Already ignored' )->output();
					continue;
				} elseif ( is_array( $processed_urls[ $image_string ] ) ) {
					$response_code_escaped = wp_kses(
						$processed_urls[ $image_string ]['response_code'],
						wp_kses_allowed_html( 'post' )
					);
					if ( 200 === $response_code_escaped ) {
						$response_code_escaped = ConsoleColor::green( $response_code_escaped )->get();
					} else {
						$response_code_escaped = ConsoleColor::yellow( $response_code_escaped )->get();
					}

					// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					echo ConsoleColor::blue( 'Already processed: ' )->get() . "$response_code_escaped\n";
				}

				ConsoleColor::green( 'Already processed as Attachment ID:' )->bright_green( $processed_urls[ $image_string ] )->output();
				continue;
			}

			$trimmed_image_string = trim( $image_string );
			if ( $trimmed_image_string !== $image_string ) {
				ConsoleColor::yellow( 'String was trimmed..' )->output();
				$image_string = $trimmed_image_string;
			}

			if ( filter_var( $image_string, FILTER_VALIDATE_URL ) ) {
				ConsoleColor::white( 'Is URL' )->output();

				// Case insensitive search for lines ending with 'jpg', 'jpeg', or 'png'.
				// These need to have a '.' prepended.
				preg_match( '/(?<!\.)(jpg|jpeg|png)$/i', $image_string, $matches );

				if ( ! empty( $matches ) ) {
					ConsoleColor::yellow( 'Having to replace extension..' )->output();
					$image_string = str_replace( $matches[0], '.' . $matches[0], $image_string );
				}

				$image_string_decoded = urldecode( $image_string );

				if ( $image_string_decoded !== $image_string ) {
					ConsoleColor::white( 'Decoded URL' )->bright_white( $image_string_decoded )->output();
					$image_string = $image_string_decoded;
				}

				if ( ! $this->is_supported_file_type( $image_string ) ) {
					ConsoleColor::bright_magenta( 'Ignoring unsupported file type.' )->output();
					$processed_urls[ $image_string ] = false;

					// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_file_put_contents
					file_put_contents( $processed_urls_path, wp_json_encode( $processed_urls, JSON_PRETTY_PRINT ) );
					continue;
				}

				if ( str_contains( $image_string, 'blogger.googleusercontent.com' ) ) {
					ConsoleColor::bright_magenta( 'Ignoring blogger domain.' )->output();
					$processed_urls[ $image_string ] = false;
				...
				if ( str_contains( $image_string, 'i0.wp.com/' ) ) {
					$new_image_string                    = str_replace( 'i0.wp.com/', '', $image_string );
					$new_image_string_with_color_escaped = str_replace(
						'i0.wp.com/',
						ConsoleColor::underlined_bright_red( 'i0.wp.com/' )->get(),
						wp_kses( $image_string, wp_kses_allowed_html( 'post' ) )
					);

					// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					echo ConsoleColor::white( 'String altered:' )->get() . "$new_image_string_with_color_escaped\n";
					$image_string = $new_image_string;
				}

				$get = wp_remote_head( $image_string );

				if ( is_wp_error( $get ) ) {
					$image_string = str_replace( 'https://', 'http://', $image_string );
					$get          = wp_remote_head( $image_string );
				}

				if ( is_wp_error( $get ) ) {
					ConsoleColor::red( 'Error' )->bright_red( $get->get_error_message() )->output();
					$this->search_for_relative_string( $image_string, $processed_urls, $processed_urls_path, $ignore_post_ids, $ignore_post_ids_placeholders );
					continue;
				}

				$get_status_code = wp_remote_retrieve_response_code( $get );

				if ( 404 === $get_status_code && str_contains( $image_string, '?' ) ) {
					$image_string = preg_replace( '/\?.*/', '', $image_string );
					ConsoleColor::white( 'Removing query string' )->bright_white( $image_string )->output();
				}

				if ( 200 === $get_status_code ) {
					ConsoleColor::green( '200' )->output();
				} elseif ( 404 === $get_status_code ) {
					ConsoleColor::red( '404' )->output();
				} else {
					ConsoleColor::yellow( $get_status_code )->output();
				}

				if ( $get_status_code >= 400 ) {
					if (
						str_contains( $image_string, 'googleusercontent.com' ) ||
						str_contains( $image_string, 'githubusercontent.com' ) ||
						str_contains( $image_string, 'google.com' )
					) {
						ConsoleColor::bright_magenta( 'Ignoring domain.' )->output();
						$processed_urls[ $image_string ] = false;
					...
					$processed_urls[ $image_string ] = [ 'response_code' => $get_status_code ];
	...
	/**
	 * This function is used when searching for a relative file path like '/media/filename.jpg' or '/default/server/filename.txt'.
	 *
	 * @param string $image_string The original file path.
	 * @param array  $processed_urls A data object containing processed URLs and file paths.
	 * @param string $processed_urls_path The path to the file containing the processed URLs.
	 * @param array  $ignore_post_ids List of Post IDs to ignore.
	 * @param string $ignore_post_ids_placeholders Placeholder string for prepared SQL statements.
	 *
	 * @return bool|null
	 */
	private function search_for_relative_string( string $image_string, array &$processed_urls, string $processed_urls_path, array $ignore_post_ids, string $ignore_post_ids_placeholders ): ?bool {
		if ( ! $this->is_supported_file_type( $image_string ) ) {
			ConsoleColor::bright_magenta( 'Ignoring unsupported file type.' )->output();
			$processed_urls[ $image_string ] = false;
		...
		$exploded = explode( '/', $image_string );
		$filename = array_pop( $exploded );
		$filename = preg_replace( '/\?.*/', '', $filename ); // Remove query string, jic.
		ConsoleColor::white( 'Filename:' )->bright_white( Colors::decolorize( $filename ) )->output();
		$file_path = $this->get_file_from_media_cache( $filename );

		if ( ! empty( $file_path ) ) {
			ConsoleColor::cyan( 'Found in media cache' )->bright_cyan( $file_path )->output();
			$this->replace_image_url_in_content( $image_string, $file_path, $processed_urls, $processed_urls_path, $ignore_post_ids, $ignore_post_ids_placeholders );

			return null;
		}

		// If not found, check to see if filename has dimensions, and remove it. e.g. -300x200.jpg.
		$filename_without_dimensions = preg_replace( '/-[0-9]+x[0-9]+\./', '.', $filename );
		if ( $filename_without_dimensions !== $filename ) {
			ConsoleColor::white( 'Searching again with:' )->bright_white( Colors::decolorize( $filename_without_dimensions ) )->output();
			$file_path = $this->get_file_from_media_cache( $filename_without_dimensions );

			if ( ! empty( $file_path ) ) {
				ConsoleColor::cyan( 'Found in media cache' )->bright_cyan( $file_path )->output();
				$this->replace_image_url_in_content(
					$image_string,
					$file_path,
					$processed_urls,
					$processed_urls_path,
					$ignore_post_ids,
					$ignore_post_ids_placeholders
				);
		...
		$filename_url_decoded = urldecode( $filename );
		if ( $filename_url_decoded !== $filename ) {
			ConsoleColor::white( 'Searching again with:' )->bright_white( Colors::decolorize( $filename_url_decoded ) )->output();
			$file_path = $this->get_file_from_media_cache( $filename_url_decoded );
		...
		ConsoleColor::red( 'Not Found.' )->output();
		$processed_urls[ $image_string ] = false;
	...
	/**
	 * Replaces the provided image URL in the content of all posts that contain it.
	 *
	 * @param string $original_file_string The original string from the log file with broken URLs/references.
	 * @param string $image_path The path of the image file in the local media cache.
	 * @param array  $processed_urls The array of processed URLs.
	 * @param string $processed_urls_path The path to the file containing the processed URLs.
	 * @param array  $ignore_post_ids The array of post IDs to ignore.
	 * @param string $ignore_post_ids_placeholders The placeholders for the post IDs to ignore.
	 *
	 * @return void
	 */
	private function replace_image_url_in_content( string $original_file_string, string $image_path, array &$processed_urls, string $processed_urls_path, array $ignore_post_ids = [], string $ignore_post_ids_placeholders = '' ): void {
		global $wpdb;

		$attachment_id = $this->attachments->import_external_file( $image_path );

		if ( is_wp_error( $attachment_id ) ) {
			ConsoleColor::red( $attachment_id->get_error_message() )->output();
			$processed_urls[ $original_file_string ] = false;

			// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_file_put_contents
			file_put_contents( $processed_urls_path, wp_json_encode( $processed_urls, JSON_PRETTY_PRINT ) );
		} else {
			// No caching is required, and SQL has been prepared and escaped.
			// phpcs:disable
				...
				$wpdb->prepare(
					"SELECT ID, post_content 
								...
								WHERE post_type = 'post' 
								  ...
								  AND ID NOT IN ( $ignore_post_ids_placeholders )",
					'%' . $wpdb->esc_like( $original_file_string ) . '%',
					...$ignore_post_ids
				)
			);
			// phpcs:enable

			$new_image = get_attached_file( $attachment_id );

			if ( filter_var( $original_file_string, FILTER_VALIDATE_URL ) ) {
				$new_image = wp_get_attachment_url( $attachment_id );
			} else {
				$base_dir  = str_replace( '/wp-content', '', WP_CONTENT_DIR );
				$new_image = str_replace( $base_dir, '', $new_image );
			}

			ConsoleColor::yellow( '# of posts with old URL:' )->bright_yellow( count( $affected_posts ) )->output();
			foreach ( $affected_posts as $affected_post ) {
				ConsoleColor::white( $affected_post->ID )->output();
				$new_post_content = str_replace( $original_file_string, $new_image, $affected_post->post_content );
				...
				if ( false === $update ) {
					ConsoleColor::red( 'Failed to update post content' )->output();
				} else {
					ConsoleColor::green( 'Updated post content' )->output();
				}
			}

			$processed_urls[ $original_file_string ] = $attachment_id;
	...
	/**
	 * Searches for the given file in the local media cache.
	 *
	 * @param string $filename The name of the file to search for.
	 *
	 * @return string|null
	 */
	private function get_file_from_media_cache( string $filename ): ?string {
		if ( file_exists( WP_CONTENT_DIR . "/Media/$filename" ) ) {
			return WP_CONTENT_DIR . "/Media/$filename";
		}

		if ( file_exists( WP_CONTENT_DIR . "/Media_2022_09_2023/$filename" ) ) {
			return WP_CONTENT_DIR . "/Media_2022_09_2023/$filename";
		}

		return $this->attachments->find_in_media_library( $filename );
	}

	/**
	 * This function looks at the given string to determine if it ends with an unsupported file extension.
	 * This helps in avoiding attempting to upload this file if it is unsupported.
	 *
	 * @param string $filename The filename to check.
	 *
	 * @return bool
	 */
	private function is_supported_file_type( string $filename ): bool {
		$unsupported_file_types = [
			'svg',
			'jfif',
		];

		foreach ( $unsupported_file_types as $unsupported_file_type ) {
			if ( str_ends_with( $filename, ".$unsupported_file_type" ) ) {
				return false;
			}
		}
