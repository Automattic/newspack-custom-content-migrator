...
class SoccerAmericaMigrator implements InterfaceCommand
{
	...
	public function register_commands() {
		...
		WP_CLI::add_command(
			'newspack-content-migrator soccer-america-add-article-redirects',
			[$this, 'cmd_add_article_redirects'],
			[
				'shortdesc' => 'Updates article slugs to better match the original slugs.',
				'synopsis'  => [
					[
						'type'        => 'flag',
						'name'        => 'dry-run',
	...
	/**
	 * Callable for `newspack-content-migrator soccer-america-add-article-redirects`
	 */
	public function cmd_add_article_redirects( $args, $assoc_args ) {
		$this->log( 'Adding article redirects.', 'line' );
		...
		// Get all the posts without redirects.
		$posts = get_posts(
			[
				'post_type'      => 'post',
				'posts_per_page' => -1,
				...
				'meta_query'     => [
					'relation' => 'AND',
					[
						'key'     => self::META_PREFIX . 'redirected',
						'compare' => 'NOT EXISTS',
					],
					[
						'key'     => self::META_PREFIX . 'shortlink',
						'compare' => 'EXISTS',
					],
				],
			]
		...
		// Start the CLI progress bar.
		$progress = \WP_CLI\Utils\make_progress_bar(
			sprintf( 'Add redirects for %d posts', count( $posts ) ),
			count( $posts )
		);
		...
		foreach ( $posts as $post ) {
			...
			// Get the import data from meta.
			$shortlink = get_post_meta( $post->ID, self::META_PREFIX . 'shortlink', true );
			if ( ! $shortlink || 'NULL' === $shortlink ) {
				$this->log( sprintf( 'No shortlink found for post %d', $post->ID ) );
				continue;
			}

			// Ping the shortlink to get the redirect URL.
			$response = wp_remote_head( $shortlink );
			if ( is_wp_error( $response ) ) {
				$this->log( sprintf( 'Failed to ping shortlink %s', $shortlink ) );
				continue;
			}
			...
			if ( ! $redirect_url ) {
				$this->log( sprintf( 'Failed to get redirect URL for shortlink %s', $shortlink ) );
				continue;
			}

			// Extract the redirect URL.
			$from = str_replace( 'http://www.socceramerica.com/', $site_url, $redirect_url );

			// Add the redirect.
			$redirect = ( $this->dryrun ) ? true : $this->redirection->create_redirection_rule(
				sprintf( 'Original URL redirect for post ID %d', $post->ID ),
				$from,
				get_permalink( $post->ID ),
			);
			if ( ! $redirect ) {
				$this->log( sprintf( 'Failed to create redirect for post ID %d', $post->ID ) );
				continue;
			}

			// Add a meta flag to show we've added the redirect.
			if ( ! $this->dryrun ) {
				add_post_meta( $post->ID, self::META_PREFIX . 'redirected', '1' );
			}

			$this->log( sprintf( 'Adding redirect took %d seconds', microtime( true ) - $start_time ) );
		}
