...
class LatinFinanceMigrator implements InterfaceCommand {
			...
			[
				...
				'synopsis'  => [
					[
						'type'        => 'assoc',
						'name'        => 'import-batch',
						'description' => 'The import batch to run: newsletters (default) | missing-newsletters',
						'optional'    => true,
						'repeating'   => false,
						'default'     => 'newsletters',
					],
	...
	public function cmd_export_from_mailchimp( $pos_args, $assoc_args ) {
		...
		if( ! in_array( $import_batch, array( 'newsletters', 'missing-newsletters' ) ) ) {
		...
		foreach( $files as $filepath ) {
			...
			if( 'missing-newsletters' == $import_batch && in_array( $date, $hack_data_mailchimp_dates ) ) continue;
