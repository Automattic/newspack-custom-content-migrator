...
class WooCommOrdersAndSubscriptionsMigrator implements InterfaceCommand {
	...
	public function cmd_wc_order_migrate( $args, $assoc_args ) {
		...
		foreach ( $source_order_ids as $key_source_order_ids => $source_order_id ) {
			...
			try {
				$this->migrate_order_or_subscription( $table_prefix_source, $table_prefix_destination, $source_order_id, null );
			} catch ( \Exception $e ) {
	 ...
	 */
	private function migrate_order_or_subscription( $table_prefix_source, $table_prefix_destination, $source_order_id = null, $source_subscription_id = null ) {
		global $wpdb;
