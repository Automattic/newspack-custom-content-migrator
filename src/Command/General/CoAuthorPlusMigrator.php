...
class CoAuthorPlusMigrator implements InterfaceCommand {
	...
	public function register_commands() {
		...
		WP_CLI::add_command(
			'newspack-content-migrator co-authors-import-posts-gas',
	...
	/**
	 * Works with the .php file saved by self::cmd_export_posts_gas() and assigns the Guest Authors to the posts.
	 * Creates a log of GAs it created anew.
	 *
	...
	public function cmd_import_posts_gas( array $args, array $assoc_args ) {
		...
		if ( ! file_exists( $php_file ) ) {
			WP_CLI::error( sprintf( 'File %s does not exist.', $php_file ) );
		}
		...
		foreach ( $posts_gas as $post_id => $ga_display_names ) {
			...
			foreach ( $ga_display_names as $ga_display_name ) {
				$guest_author = $this->coauthorsplus_logic->get_guest_author_by_display_name( $ga_display_name );
				if ( ! $guest_author ) {
					$ga_id = $this->coauthorsplus_logic->create_guest_author( [ 'display_name' => $ga_display_name ] );
	...
	public function cmd_delete_authors_with_zero_posts( $args, $assoc_args ) {
		...
		foreach ( $all_gas as $ga ) {
			if ( 0 === count( $this->coauthorsplus_logic->get_all_posts_for_guest_author( $ga->ID ) ) ) {
				$gas_to_delete[] = $ga;
			}
		}
